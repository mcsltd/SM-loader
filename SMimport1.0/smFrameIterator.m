...
classdef smFrameIterator
    ...
    methods
        function obj = smFrameIterator(siginfo, mex_support)
            obj.siginfo = siginfo;
            obj.pos = 1;
            obj.size = 0;
        ...
        function samples = decode_with_mex(obj, frame)
            ...
            samples = smdecode(frame_data, frame.size, ...
                ...
                obj.siginfo{frame.channel}.uv_per_bit);
        end
        ...
        %     i = 1;
        %     try
        %     while i <= length(frame_data) && tick_counter < frame.size
        ...
        %                 i = i + 2;
        %                 if MAX_BYTE == 3
        %                    diff = idivide(typecast([obj.block.data(i:i+2), int8(0)],'int32'), 256);
        %                    i = i + 3;
        %                 else
